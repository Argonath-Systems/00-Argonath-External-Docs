...
*   `anchor-*`: Maps to Hytale anchors (e.g., `anchor-left`, `anchor-top`, `anchor-width`, `anchor-height`, `anchor-min-width`, `anchor-max-width`, `anchor-min-height`, `anchor-max-height`).
*   `min-height`, `max-height`: Height constraints (aliases for `anchor-min-height`, `anchor-max-height`).
*   `padding`, `padding-*`: Internal spacing (e.g., `padding: 8`, `padding-left: 10`).
*   `margin`, `margin-*`: External spacing (e.g., `margin: 8`, `margin-top: 16`). Applied as anchor offsets.
*   `gap`, `row-gap`, `column-gap`: Spacing between child elements in flex containers.
*   `scale`, `ui-scale`, `transform-scale`: Proportional scaling factor for the element (1.0 = 100%, 1.5 = 150%). Scales anchor values and font sizes.
*   `data-open-modal`: Opens the specified modal by ID when element is activated.
*   `data-close-modal`: Closes the specified modal by ID, or all modals if empty.
*   `data-toggle-modal`: Toggles the specified modal by ID between open and closed.
